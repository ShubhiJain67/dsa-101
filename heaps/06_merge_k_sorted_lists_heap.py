...
class Solution:
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        ...
        # return self.mergeHeaps(lists)
        return self.mergeHeapsOptimised(lists)

    
    def mergeHeapsOptimised(self, lists):
        heap = []
        index = 0
        for lst in lists:
            if lst is None:
        ...
        ptr = None
        while heap:
            value, index, curr = heapq.heappop(heap)
            if head == None:
                head = curr
                ptr = curr
            else:
    ...
    def mergeHeaps(self, lists):
        ...
        for index in range(len(lists)):
            ...
            while l:
                ...
                l = l.next
                curr.next = None
                heap.append([curr.val, index, count, curr])
                count += 1
        heapq.heapify(heap)
        head = None
        ptr = None
        while heap:
            _, _, _, curr = heapq.heappop(heap)
            if head == None:
                head = curr
                ptr = curr
            else:
